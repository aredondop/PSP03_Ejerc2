...
public class PSP03_Ejer2_Servidor {
    ...
    private static void manejarConexion(DataInputStream dis, DataOutputStream dos) throws IOException {
        ...
        do {
            ...
            existeFichero = verificarExistenciaArchivo(rutaFichero, dos);
        } while (!existeFichero);
    ...
    /**
     * Verifica la existencia del archivo en la ruta especificada y envía el resultado al cliente.
     ...
     */
    private static boolean verificarExistenciaArchivo(String rutaFichero, DataOutputStream dos) throws IOException {
        Path archivoPath = Paths.get(rutaFichero);
        boolean existeFichero = Files.exists(archivoPath);

        dos.writeBoolean(existeFichero);
    ...
    private static void enviarArchivo(String rutaFichero, DataOutputStream dos) throws IOException {
        ...
        try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(archivoPath.toFile()))) {
            byte[] buffer = new byte[8192]; // 8KB buffer (ajusta el tamaño según sea necesario)
            int bytesRead;
